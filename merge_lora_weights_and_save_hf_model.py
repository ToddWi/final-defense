...
# 디렉토리에서 분할된 가중치를 병합
def load_and_merge_state_dict(model_dir):
    # index.json 파일 읽기
    ...
    # 각 .bin 파일 병합
    for bin_file in set(index_data["weight_map"].values()):
        file_path = os.path.join(model_dir, bin_file)
...
def main(args):
    ...
    if os.path.isdir(args.weight):
        # 디렉토리 처리 (.bin 파일 병합하여 torch.load)
        state_dict = load_and_merge_state_dict(args.weight)
        model.load_state_dict(state_dict, strict=True)
        
    elif os.path.isfile(args.weight):
        # 단일 파일 처리 (예: torch.load 사용)
        state_dict = torch.load(args.weight, map_location="cpu")
        model.load_state_dict(state_dict, strict=True)
    else:
        raise ValueError(f"Invalid path: {args.weight}. Must be a file or a directory.")
